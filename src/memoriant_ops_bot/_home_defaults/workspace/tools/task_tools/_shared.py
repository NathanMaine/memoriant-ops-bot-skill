...
def detect_agent_name() -> str:
    ...
    Sub-agent tools live at ``~/.mops/agents/<name>/workspace/tools/task_tools/``.
    Main agent tools live at ``~/.mops/workspace/tools/task_tools/``.
    ...
    # Fallback to env var
    return os.environ.get("MOPS_AGENT_NAME", "main")
...
def post_json(url: str, body: dict[str, object], *, timeout: int = 300) -> dict[str, object]:
    ...
    try:
        ...
        print("Make sure the Mops bot is running with tasks enabled.", file=sys.stderr)
        sys.exit(1)
