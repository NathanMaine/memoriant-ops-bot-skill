...
Environment variables MOPS_AGENT_NAME, MOPS_INTERAGENT_PORT, and
MOPS_INTERAGENT_HOST are automatically set by the Memoriant Ops framework.
...
def main() -> None:
    ...
    target = args[0]
    message = args[1]
    port = os.environ.get("MOPS_INTERAGENT_PORT", "8799")
    host = os.environ.get("MOPS_INTERAGENT_HOST", "127.0.0.1")
    sender = os.environ.get("MOPS_AGENT_NAME", "unknown")
    ...
    chat_id = os.environ.get("MOPS_CHAT_ID", "")
    topic_id = os.environ.get("MOPS_TOPIC_ID", "")
    if chat_id:
    ...
    try:
        ...
        print(
            "Make sure the Mops supervisor is running with multi-agent support.", file=sys.stderr
        )
        sys.exit(1)
