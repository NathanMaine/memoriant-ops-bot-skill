...
def _shared_path() -> Path:
    ...
    Priority:
    1. MOPS_SHARED_MEMORY_PATH env var (set by framework)
    2. MOPS_HOME / SHAREDMEMORY.md (works for main agent)
    3. Navigate up from sub-agent home: agents/<name>/ -> ../../SHAREDMEMORY.md
    """
    env_path = os.environ.get("MOPS_SHARED_MEMORY_PATH")
    if env_path:
        return Path(env_path)

    home = Path(os.environ.get("MOPS_HOME", str(Path.home() / ".mops")))
    direct = home / "SHAREDMEMORY.md"
    if direct.is_file():
        return direct

    # Sub-agent: home is ~/.mops/agents/<name>/, shared is at ~/.mops/SHAREDMEMORY.md
