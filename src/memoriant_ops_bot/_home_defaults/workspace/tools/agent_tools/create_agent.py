...
def _agents_path() -> Path:
    ...
    Sub-agents have MOPS_HOME = ~/.mops/agents/<name>/, so we navigate
    up to the main home. Main agent's MOPS_HOME points directly to ~/.mops/.
    """
    import os
...
def _main_home() -> Path:
    """Resolve the main agent's MOPS_HOME."""
    import os

    home = Path(os.environ.get("MOPS_HOME", str(Path.home() / ".mops")))
    if home.name != ".mops" and (home.parent.parent / "config").is_dir():
