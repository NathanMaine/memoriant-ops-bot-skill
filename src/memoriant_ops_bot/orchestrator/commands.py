...
from memoriant_ops_bot.text.response_format import SEP, fmt, new_session_text
...
def _resolve_log_path(orch: Orchestrator) -> Path:
    ...
    log in the main mops home (parent of ``agents/<name>``).
    """
    log_path = orch.paths.logs_dir / "agent.log"
    if not log_path.exists():
        main_logs = orch.paths.mops_home.parent.parent / "logs" / "agent.log"
        if main_logs.exists():
            return main_logs
    return log_path
