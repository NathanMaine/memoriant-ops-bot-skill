...
def _docker_skill_resync(paths: MopsPaths) -> None:
...
async def create_orchestrator(
    ...
    *,
    ...
    if agent_name == "main":
        os.environ["MOPS_HOME"] = str(paths.mops_home)
...
async def start_api_server(
    ...
    try:
        ...
        logger.warning(
            "API server enabled but PyNaCl is not installed. Install with: pip install memoriant-ops-bot[api]"
        )
        return
...
async def shutdown(orch: Orchestrator) -> None:
    ...
    await asyncio.to_thread(cleanup_mops_links, orch._paths)
