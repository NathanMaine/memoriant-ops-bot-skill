...
from memoriant_ops_bot.bus.bus import MessageBus
from memoriant_ops_bot.bus.lock_pool import LockPool
from memoriant_ops_bot.commands import BOT_COMMANDS as _COMMAND_DEFS
from memoriant_ops_bot.commands import MULTIAGENT_SUB_COMMANDS as _MA_SUB_DEFS
from memoriant_ops_bot.config import AgentConfig
from memoriant_ops_bot.files.allowed_roots import resolve_allowed_roots
from memoriant_ops_bot.i18n import t
from memoriant_ops_bot.infra.restart import EXIT_RESTART, consume_restart_marker
from memoriant_ops_bot.infra.updater import UpdateObserver
from memoriant_ops_bot.infra.version import VersionInfo, get_current_version
from memoriant_ops_bot.log_context import set_log_context
from memoriant_ops_bot.messenger.notifications import NotificationService
from memoriant_ops_bot.messenger.telegram.callbacks import (
    edit_selector_response,
    mark_button_choice,
    parse_ns_callback,
)
from memoriant_ops_bot.messenger.telegram.chat_tracker import ChatRecord, ChatTracker
from memoriant_ops_bot.messenger.telegram.file_browser import (
    file_browser_start,
    handle_file_browser_callback,
    is_file_browser_callback,
)
from memoriant_ops_bot.messenger.telegram.formatting import markdown_to_telegram_html
from memoriant_ops_bot.messenger.telegram.handlers import (
    handle_abort,
    handle_abort_all,
    handle_command,
    handle_interrupt,
...
from memoriant_ops_bot.messenger.telegram.media import (
    has_media,
...
from memoriant_ops_bot.messenger.telegram.message_dispatch import (
...
from memoriant_ops_bot.messenger.telegram.sender import SendRichOpts, send_rich
from memoriant_ops_bot.messenger.telegram.sender import (
    send_files_from_text as _send_files_from_text,
)
from memoriant_ops_bot.messenger.telegram.topic import (
    TopicNameCache,
    get_session_key,
    get_thread_id,
)
from memoriant_ops_bot.messenger.telegram.typing import TypingContext as _TypingContext
...
from memoriant_ops_bot.multiagent.bus import AsyncInterAgentResult
from memoriant_ops_bot.session.key import SessionKey
from memoriant_ops_bot.tasks.models import TaskResult
from memoriant_ops_bot.text.response_format import SEP, fmt
...
_WELCOME_IMAGE = Path(__file__).resolve().parent / "mops_images" / "welcome.png"
_CAPTION_LIMIT = 1024
...
def _rebuild_commands() -> None:
    ...
    from memoriant_ops_bot.commands import get_bot_commands, get_multiagent_sub_commands
...
def _build_help_text() -> str:
    return fmt(
        ...
        "**Infrastructure**\n"
        "/spark -- DGX Spark status\n"
        "/nas -- NAS disk usage\n"
        "/health -- Full system health\n"
        "/braintrust -- Search expert knowledge\n"
        "/report -- Ops report summary\n"
        "/models -- Ollama model list\n"
        "/experts -- Brain Trust experts\n"
        "/chunks -- Qdrant stats",
...
class TelegramBot:
    ...
    def __init__(
        ...
        *,
        ...
        from memoriant_ops_bot.messenger.telegram.transport import TelegramTransport
    ...
    async def _on_startup(self) -> None:
        from memoriant_ops_bot.messenger.telegram.startup import run_startup
    ...
    async def _send_join_notification(self, chat_id: int) -> None:
        ...
        if not text:
            return
        from memoriant_ops_bot.messenger.telegram.sender import _send_text_chunks
    ...
    async def _show_welcome(self, message: Message) -> None:
        ...
        from memoriant_ops_bot.cli.auth import check_all_auth
            ...
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="GitHub", url="https://github.com/NathanMaine/memoriant-ops-bot"
                    ),
                    InlineKeyboardButton(
                        text="Changelog",
                        url="https://github.com/NathanMaine/memoriant-ops-bot/releases",
                    ),
                ],
                [InlineKeyboardButton(text="PyPI", url="https://pypi.org/project/mops/")],
            ],
        )
        await send_rich(
            self._bot,
    ...
    async def _on_showfiles(self, message: Message) -> None:
        """Handle /showfiles: interactive file browser for ~/.mops."""
    ...
    async def _on_memoriant_command(self, message: Message) -> None:
        """Handle Memoriant-specific infrastructure commands."""
        if self._is_for_others(message):
            return
        if not message.text:
            return
        from memoriant_ops_bot.commands.memoriant_commands import dispatch_memoriant_command
        ...
        chat_id = message.chat.id
        thread_id = get_thread_id(message)
        async with _TypingContext(self._bot, chat_id, thread_id=thread_id):
            result = await dispatch_memoriant_command(cmd, args)
        if result:
            await send_rich(
                self._bot,
                chat_id,
    ...
    async def _on_forum_topic_created(self, message: Message) -> None:
        ...
        from memoriant_ops_bot.messenger.telegram.topic import get_topic_name_from_message
    ...
    async def _on_session(self, message: Message) -> None:
        ...
        try:
            if session_followup:
                ...
                from memoriant_ops_bot.orchestrator.core import NamedSessionRequest
    ...
    async def _on_restart(self, message: Message) -> None:
        if self._config.group_mention_only and not self._is_addressed(message):
            return
        from memoriant_ops_bot.infra.restart import write_restart_sentinel

        chat_id = message.chat.id
        paths = self._orch.paths
        sentinel = paths.mops_home / "restart-sentinel.json"
    ...
    async def _route_special_callback(
        ...
        from memoriant_ops_bot.orchestrator.selectors.cron_selector import is_cron_selector_callback
    ...
    async def _handle_model_selector(self, key: SessionKey, message_id: int, data: str) -> None:
        ...
        from memoriant_ops_bot.orchestrator.selectors.model_selector import handle_model_callback
    ...
    async def _handle_cron_selector(self, chat_id: int, message_id: int, data: str) -> None:
        ...
        from memoriant_ops_bot.orchestrator.selectors.cron_selector import handle_cron_callback
    ...
    async def _handle_task_selector(self, chat_id: int, message_id: int, data: str) -> None:
        ...
        from memoriant_ops_bot.orchestrator.selectors.task_selector import handle_task_callback
    ...
    async def _handle_ns_callback(
        ...
        async with self._sequential.get_lock(key.lock_key):
            if self._config.streaming.enabled:
                from memoriant_ops_bot.orchestrator.flows import named_session_streaming

                result = await named_session_streaming(self._orch, key, session_name, label)
            else:
                from memoriant_ops_bot.orchestrator.flows import named_session_flow
    ...
    async def on_async_interagent_result(self, result: AsyncInterAgentResult) -> None:
        ...
        from memoriant_ops_bot.bus.adapters import from_interagent_result
    ...
    async def on_task_result(self, result: TaskResult) -> None:
        ...
        from memoriant_ops_bot.bus.adapters import from_task_result
    ...
    async def on_task_question(
        ...
        from memoriant_ops_bot.bus.adapters import from_task_question
    ...
    async def _handle_webhook_wake(self, chat_id: int, prompt: str) -> str | None:
        ...
        from memoriant_ops_bot.bus.envelope import LockMode
        ...
        from memoriant_ops_bot.bus.adapters import from_webhook_wake
    ...
    async def _on_update_available(self, info: VersionInfo) -> None:
        ...
        from memoriant_ops_bot.messenger.telegram.upgrade_handler import on_update_available
    ...
    async def _handle_upgrade_callback(
        ...
        from memoriant_ops_bot.messenger.telegram.upgrade_handler import handle_upgrade_callback
    ...
    async def _watch_restart_marker(self) -> None:
        ...
        paths = self._orch.paths
        marker = paths.mops_home / "restart-requested"
        try:
            while True:
