"""Interactive file browser for the ~/.mops directory.

Renders the mops home directory as a navigable inline-keyboard tree.
...
from memoriant_ops_bot.files.browser import list_directory
from memoriant_ops_bot.i18n import t
from memoriant_ops_bot.security.paths import is_path_safe
...
async def file_browser_start(paths: MopsPaths) -> tuple[str, InlineKeyboardMarkup]:
...
async def handle_file_browser_callback(
    ...
    if data.startswith(SF_FILE_PREFIX):
        ...
        abs_dir = (paths.mops_home / rel).resolve() if rel else paths.mops_home.resolve()
...
def _build_view(paths: MopsPaths, rel: str) -> tuple[str, InlineKeyboardMarkup]:
    """Build the text + keyboard for a directory listing."""
    base = paths.mops_home.resolve()
    ...
    display_path = f"~/.mops/{rel}" if rel else "~/.mops/"
