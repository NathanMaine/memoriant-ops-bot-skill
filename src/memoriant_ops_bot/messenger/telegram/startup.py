...
from memoriant_ops_bot.infra.restart import consume_restart_sentinel
from memoriant_ops_bot.infra.updater import UpdateObserver, consume_upgrade_sentinel
...
async def _handle_restart_sentinel(bot: TelegramBot) -> dict[str, object] | None:
    ...
    sentinel_path = bot._orch.paths.mops_home / "restart-sentinel.json"
...
async def _handle_recovery(bot: TelegramBot, sentinel: dict[str, object] | None) -> None:
    ...
    upgrade = await asyncio.to_thread(consume_upgrade_sentinel, bot._orch.paths.mops_home)
    if upgrade:
    ...
    from memoriant_ops_bot.infra.startup_state import detect_startup_kind, save_startup_state
    from memoriant_ops_bot.text.response_format import startup_notification_text
    ...
    from memoriant_ops_bot.infra.recovery import RecoveryPlanner
    from memoriant_ops_bot.text.response_format import recovery_notification_text
...
async def _run_primary_startup(bot: TelegramBot) -> None:
    ...
    from memoriant_ops_bot.messenger.telegram.chat_tracker import ChatTracker
    ...
    from memoriant_ops_bot.infra.install import is_upgradeable
