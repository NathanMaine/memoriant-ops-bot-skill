...
from memoriant_ops_bot.commands import BOT_COMMANDS, MULTIAGENT_SUB_COMMANDS
...
from memoriant_ops_bot.messenger.commands import classify_command
from memoriant_ops_bot.messenger.matrix.buttons import ButtonTracker
from memoriant_ops_bot.messenger.matrix.credentials import login_or_restore
from memoriant_ops_bot.messenger.matrix.id_map import MatrixIdMap
from memoriant_ops_bot.messenger.matrix.sender import send_rich as matrix_send_rich
from memoriant_ops_bot.messenger.matrix.streaming import MatrixStreamEditor
from memoriant_ops_bot.messenger.matrix.typing import MatrixTypingContext
...
def _expand_marker(mops_home: str) -> Path:
    """Return the restart-marker path (sync, no I/O)."""
    return Path(mops_home).expanduser() / "restart-requested"
...
class MatrixBot:
        ...
        *,
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install with: pip install 'memoriant-ops-bot[matrix]'"
            ) from None
        ...
        from memoriant_ops_bot.messenger.matrix.transport import MatrixTransport
    ...
    async def run(self) -> int:
        ...
        from memoriant_ops_bot.messenger.matrix.startup import run_matrix_startup
    ...
    async def _on_media(self, room: MatrixRoom | object, event: RoomMessageMedia | object) -> None:
        ...
        from memoriant_ops_bot.messenger.matrix.media import resolve_matrix_media
    ...
    async def _cmd_showfiles(
        ...
        from memoriant_ops_bot.messenger.matrix.file_browser import format_file_listing
    ...
    def _maybe_append_footer(self, result: object) -> None:
        ...
        if not self._config.scene.technical_footer or not result.model_name:
            return
        from memoriant_ops_bot.text.response_format import format_technical_footer
    ...
    async def _send_selector_response(
        ...
        from memoriant_ops_bot.messenger.matrix.buttons import REACTION_DIGITS
        from memoriant_ops_bot.orchestrator.selectors.models import ButtonGrid
    ...
    async def _handle_button_callback(
        ...
        from memoriant_ops_bot.messenger.callback_router import route_callback
    ...
    async def _handle_upgrade_callback(self, room_id: str, data: str) -> None:
        ...
        if data.startswith("upg:cl:"):
            ...
            if not version:
                return
            from memoriant_ops_bot.infra.version import fetch_changelog
        ...
        if data.startswith("upg:yes:"):
            from memoriant_ops_bot.infra.restart import EXIT_RESTART, write_restart_marker
            from memoriant_ops_bot.infra.updater import perform_upgrade_pipeline
            from memoriant_ops_bot.infra.version import get_current_version
    ...
    async def _handle_ns_callback(self, room_id: str, key: SessionKey, data: str) -> None:
        ...
        from memoriant_ops_bot.messenger.telegram.callbacks import parse_ns_callback
