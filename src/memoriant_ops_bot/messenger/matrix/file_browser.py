...
from memoriant_ops_bot.files.browser import BROWSER_EXCLUDED_NAMES, list_directory
...
def format_file_listing(paths: MopsPaths, subdir: str = "") -> str:
    """Format a text listing of the workspace directory structure.

    Args:
        paths: Resolved workspace paths.
        subdir: Optional subdirectory relative to ``mops_home`` to list.
...
def _format_overview(paths: MopsPaths) -> str:
...
def _format_subdir(paths: MopsPaths, subdir: str) -> str:
