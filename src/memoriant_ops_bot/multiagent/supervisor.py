...
from memoriant_ops_bot.config import AgentConfig, update_config_file_async
from memoriant_ops_bot.infra.file_watcher import FileWatcher
from memoriant_ops_bot.infra.restart import EXIT_RESTART
from memoriant_ops_bot.multiagent.health import AgentHealth
from memoriant_ops_bot.multiagent.models import SubAgentConfig, merge_sub_agent_config
from memoriant_ops_bot.multiagent.registry import AgentRegistry
from memoriant_ops_bot.multiagent.stack import AgentStack
from memoriant_ops_bot.workspace.paths import resolve_paths

if TYPE_CHECKING:
    from memoriant_ops_bot.multiagent.bus import InterAgentBus
    from memoriant_ops_bot.multiagent.internal_api import InternalAgentAPI
    from memoriant_ops_bot.multiagent.shared_knowledge import SharedKnowledgeSync
...
class AgentSupervisor:
    ...
    async def start(self) -> int:
        ...
        if self._main_config.tasks.enabled:
            from memoriant_ops_bot.tasks.hub import TaskHub
            from memoriant_ops_bot.tasks.registry import TaskRegistry
        ...
        shared_path = self._main_paths.mops_home / "SHAREDMEMORY.md"
    ...
    async def _start_sub_agent(self, sub_cfg: SubAgentConfig) -> None:
        ...
        agent_home = self._main_paths.mops_home / "agents" / name
