"""Entry point: python -m memoriant_ops_bot."""
...
# tests that patch memoriant_ops_bot.__main__.<name>.
from memoriant_ops_bot.cli_commands.agents import cmd_agents as _cmd_agents
from memoriant_ops_bot.cli_commands.api_cmd import cmd_api as _cmd_api
from memoriant_ops_bot.cli_commands.docker import cmd_docker as _cmd_docker
from memoriant_ops_bot.cli_commands.install import cmd_install as _cmd_install
from memoriant_ops_bot.cli_commands.lifecycle import (
...
from memoriant_ops_bot.cli_commands.service import cmd_service as _cmd_service
from memoriant_ops_bot.cli_commands.status import (
...
from memoriant_ops_bot.infra.json_store import atomic_json_save
from memoriant_ops_bot.workspace.init import init_workspace
from memoriant_ops_bot.workspace.paths import resolve_paths
...
def load_config() -> AgentConfig:
    ...
    Resolution order:
    1. ``~/.mops/config/config.json`` (canonical location)
    ...
    if first_start:
        ...
        if example.is_file():
            ...
            defaults.pop("api", None)  # Beta: only written by `mops api enable`
...
async def run_bot(config: AgentConfig) -> int:
    ...
    from memoriant_ops_bot.infra.zombie_guard import start_zombie_guard, stop_zombie_guard
    from memoriant_ops_bot.multiagent.supervisor import AgentSupervisor

    acquire_lock(pid_file=paths.mops_home / "bot.pid", kill_existing=True)

    # Start the ZombieGuard to prevent orphaned subprocess accumulation
    terminate_event = asyncio.Event()
    guard = await start_zombie_guard(terminate_event=terminate_event)
    ...
    # Monitor the ZombieGuard terminate event
    async def _watch_terminate() -> None:
        await terminate_event.wait()
        logger.warning("ZombieGuard requested shutdown")
        _request_shutdown()

    terminate_watcher = asyncio.create_task(_watch_terminate())
    ...
    try:
        ...
        terminate_watcher.cancel()
        for sig in installed_signals:
            loop.remove_signal_handler(sig)
        await supervisor.stop_all()
        await stop_zombie_guard()
        release_lock(pid_file=paths.mops_home / "bot.pid")
    return exit_code
...
def _cmd_setup(verbose: bool) -> None:
    ...
    from memoriant_ops_bot.cli.init_wizard import run_onboarding, run_smart_reset

    _stop_bot()
    paths = resolve_paths()
    if _is_configured():
        run_smart_reset(paths.mops_home)
...
def _default_action(verbose: bool) -> None:
    ...
    if not _is_configured():
        from memoriant_ops_bot.cli.init_wizard import run_onboarding
