"""Systemd user service management for mops (Linux)."""
...
from memoriant_ops_bot.infra.service_logs import print_journal_service_logs
...
def _generate_service_unit(binary_path: str) -> str:
    ...
    return f"""\
[Unit]
Description=Memoriant Ops Bot - Remote AI operations via Telegram
...
def is_service_installed() -> bool:
    """Check if the mops service is installed."""
    return _service_path().exists()


def is_service_running() -> bool:
    """Check if the mops service is currently running."""
...
def install_service(console: Console | None = None) -> bool:
    """Install and start the mops systemd user service.
...
def uninstall_service(console: Console | None = None) -> bool:
    """Stop, disable, and remove the mops service."""
    console = ensure_console(console)
