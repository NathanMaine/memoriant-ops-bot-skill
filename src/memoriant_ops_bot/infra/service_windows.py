"""Windows Task Scheduler service management for mops."""
...
from memoriant_ops_bot.infra.service_base import (
    ensure_console,
    find_mops_binary,
...
from memoriant_ops_bot.infra.service_logs import print_file_service_logs
...
def _generate_task_xml(command: str, arguments: str = "") -> str:
    ...
    SubElement(reg, "Description").text = "Memoriant Ops Bot - Remote AI operations via Telegram"
...
def _task_xml_path() -> Path:
    ...
    return resolve_paths().mops_home / "mops_task.xml"
...
def is_service_installed() -> bool:
    """Check if the mops scheduled task exists."""
    result = _run_schtasks("/Query", "/TN", _TASK_NAME, "/FO", "LIST")
    return result.returncode == 0


def is_service_running() -> bool:
    """Check if the mops scheduled task is currently running."""
...
def install_service(console: Console | None = None) -> bool:
    """Install and start the mops scheduled task.
    ...
    # Resolve command: prefer pythonw.exe (no console window) over mops binary
    pythonw = _find_pythonw()
    if pythonw:
        command = pythonw
        arguments = "-m memoriant_ops_bot"
    else:
        binary = find_mops_binary()
        if not binary:
...
def uninstall_service(console: Console | None = None) -> bool:
    """Stop and remove the mops scheduled task."""
    console = ensure_console(console)
...
def print_service_logs(console: Console | None = None) -> None:
    ...
    Windows has no journalctl equivalent, so we tail the mops log file.
    """
    console = ensure_console(console)
