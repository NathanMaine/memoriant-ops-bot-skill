...
_MOPS_MOUNT = "/mops"
_CONTAINER_WS = f"{_MOPS_MOUNT}/workspace"
_MOUNT_PREFIX = "/mnt"
...
class DockerManager:
    ...
    def __init__(self, config: DockerConfig, paths: MopsPaths) -> None:
        self._config = config
        self._paths = paths
    ...
    async def _start_container(self, name: str, image: str) -> bool:
        # Always mount the root mops home, even when called from a sub-agent.
        ...
        # like /mops/agents/<name>/workspace.
        mops_home = self._paths.mops_home
        if mops_home.parent.name == "agents":
            mops_home = mops_home.parent.parent
        ...
        cmd: list[str] = [
            ...
            "-w",
            _CONTAINER_WS,
            # Mount the ENTIRE ~/.mops so the CLI sees all framework files.
            "-v",
            f"{mops_home}:{_MOPS_MOUNT}",
            "-e",
            f"MOPS_HOME={_MOPS_MOUNT}",
    ...
    def _env_secret_flags(self) -> list[str]:
        """Return ``-e`` flags for user secrets from ``~/.mops/.env``."""
