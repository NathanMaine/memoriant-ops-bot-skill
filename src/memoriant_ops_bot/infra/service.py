"""Platform-dispatching service management for mops.
...
if sys.platform == "win32":
    from memoriant_ops_bot.infra import service_windows as _backend
elif sys.platform == "darwin":
    from memoriant_ops_bot.infra import service_macos as _backend
else:
    from memoriant_ops_bot.infra import service_linux as _backend
...
def install_service(console: Console | None = None) -> bool:
    """Install and start the mops background service. Returns True on success."""
    return _backend.install_service(console)


def uninstall_service(console: Console | None = None) -> bool:
    """Stop and remove the mops background service."""
