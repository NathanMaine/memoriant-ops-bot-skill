...
def kill_all_mops_processes() -> int:
    """Find and force-kill remaining mops processes system-wide.

    On Windows this uses two strategies:
    1. ``tasklist`` scan for ``mops.exe`` processes
    2. PowerShell scan for ``python.exe``/``pythonw.exe`` running from the
       pipx venv directory (covers ``pythonw.exe -m memoriant_ops_bot``)
    ...
    current = os.getpid()
    killed = _kill_mops_exe_windows(current)
...
def _kill_mops_exe_windows(current_pid: int) -> int:
    ...
    for line in result.stdout.splitlines():
        ...
        if len(parts) < 2:
            continue
        name = parts[0].lower()
        if name not in ("mops.exe", "mops"):
            continue
        with contextlib.suppress(ValueError):
            pid = int(parts[1])
            if pid == current_pid or pid <= 0:
                continue
            logger.info("Killing mops process: pid=%d name=%s", pid, name)
            _run_taskkill(pid, force=True)
            killed += 1
    return killed
...
def _kill_venv_python_windows(current_pid: int) -> int:
    ...
    When mops is installed via ``pipx``, the bot runs as
    ``pythonw.exe -m memoriant_ops_bot`` inside ``~\pipx\venvs\memoriant-ops-bot\``.
        ...
        result = subprocess.run(
            [
                ...
                (
                    ...
                    " | Where-Object { $_.Path -like '*pipx*memoriant-ops-bot*' }"
