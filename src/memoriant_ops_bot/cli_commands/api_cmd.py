"""API server management CLI subcommands (``mops api ...``)."""
...
from memoriant_ops_bot.config import _BIND_ALL_INTERFACES
...
def print_api_help() -> None:
    ...
    table.add_column()
    table.add_row("mops api enable", "Enable the WebSocket API server")
    table.add_row("mops api disable", "Disable the WebSocket API server")
...
def api_install_hint() -> str:
    ...
    if mode == "pipx":
        return "pipx inject memoriant-ops-bot PyNaCl"
    return "pip install memoriant-ops-bot[api]"


def api_enable() -> None:
    """Enable the API server: check deps, write config, generate token."""
    from memoriant_ops_bot.cli_commands.docker import docker_read_config
