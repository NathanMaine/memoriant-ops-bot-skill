"""Status display CLI commands (``mops status``, ``mops help``)."""
...
def build_status_lines(status: StatusSummary, *, paths: MopsPaths) -> list[str]:
    ...
    lines.append(f"  Home:       [cyan]{paths.mops_home}[/cyan]")
...
def print_status() -> None:
    ...
    from memoriant_ops_bot.cli_commands.agents import load_agents_registry, print_agents_status
    ...
    if docker_enabled and isinstance(docker_cfg, dict):
        docker_name = str(docker_cfg.get("container_name", "mops-sandbox"))
    ...
    if pid_file.exists():
        ...
        if bot_pid is not None:
            from memoriant_ops_bot.infra.pidlock import _is_process_alive
...
def print_usage() -> None:
    ...
    table.add_column()
    table.add_row("mops", t_rich("help.mops"))
    table.add_row("mops onboarding", t_rich("help.onboarding"))
    table.add_row("mops stop", t_rich("help.stop"))
    table.add_row("mops restart", t_rich("help.restart"))
    table.add_row("mops reset", t_rich("help.reset"))
    table.add_row("mops upgrade", t_rich("help.upgrade"))
    table.add_row("mops uninstall", t_rich("help.uninstall"))
    is_macos = sys.platform == "darwin"
    svc_hint = "Task Scheduler" if is_windows() else ("launchd" if is_macos else "systemd")
    table.add_row("mops service install", t_rich("help.service_install", hint=svc_hint))
    table.add_row("mops service", t_rich("help.service"))
    table.add_row("mops agents", t_rich("help.agents"))
    table.add_row("mops docker", t_rich("help.docker"))
    table.add_row("mops api", t_rich("help.api"))
    table.add_row("mops install <extra>", t_rich("help.install"))
    table.add_row("mops status", t_rich("help.status"))
    table.add_row("mops help", t_rich("help.help"))
