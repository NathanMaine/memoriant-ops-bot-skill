...
Every path the framework needs is either a field or property of ``MopsPaths``.
"""
...
# memoriant_ops_bot/workspace/paths.py -> memoriant_ops_bot/workspace -> memoriant_ops_bot
...
class MopsPaths:
    ...
    - ``mops_home``:      User data directory (default ``~/.mops``).
    - ``home_defaults``:  Bundled template that mirrors ``mops_home`` (package-internal).
    ...
    def workspace(self) -> Path:
        return self.mops_home / "workspace"

    @property
    def config_dir(self) -> Path:
        return self.mops_home / "config"
    ...
    def sessions_path(self) -> Path:
        return self.mops_home / "sessions.json"

    @property
    def cron_jobs_path(self) -> Path:
        return self.mops_home / "cron_jobs.json"

    @property
    def webhooks_path(self) -> Path:
        return self.mops_home / "webhooks.json"

    @property
    def logs_dir(self) -> Path:
        return self.mops_home / "logs"
    ...
    def bundled_skills_dir(self) -> Path:
        """Package-internal skill directory (read-only, ships with mops)."""
    ...
    def tasks_registry_path(self) -> Path:
        ...
        return self.mops_home / "tasks.json"

    @property
    def chat_activity_path(self) -> Path:
        return self.mops_home / "chat_activity.json"

    @property
    def named_sessions_path(self) -> Path:
        return self.mops_home / "named_sessions.json"

    @property
    def startup_state_path(self) -> Path:
        return self.mops_home / "startup_state.json"

    @property
    def inflight_turns_path(self) -> Path:
        return self.mops_home / "inflight_turns.json"

    @property
    def env_file(self) -> Path:
        """User-managed ``.env`` for external API secrets."""
        return self.mops_home / ".env"
...
def resolve_paths(
    ...
    *,
    ...
    """Build MopsPaths from explicit values, env vars, or defaults.
    ...
    if effective_home is not None:
        ...
        home = (
            Path(
                os.environ.get("MOPS_HOME", str(Path.home() / ".mops")),
            )
            .expanduser()
            .resolve()
        )
    ...
    if framework_root is not None:
        ...
        env_fw = os.environ.get("MOPS_FRAMEWORK_ROOT")
    ...
    return MopsPaths(mops_home=home, home_defaults=hd, framework_root=fw)
