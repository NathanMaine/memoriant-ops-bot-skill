...
from memoriant_ops_bot.infra.atomic_io import atomic_text_save
from memoriant_ops_bot.workspace.cron_tasks import ensure_task_rule_files
...
from memoriant_ops_bot.workspace.skill_sync import sync_bundled_skills, sync_skills
...
def _sync_home_defaults(paths: MopsPaths) -> None:
    """Walk the home-defaults template and copy to ``mops_home``.

    The template at ``<repo>/workspace/`` mirrors ``~/.mops/`` exactly.
    Zone rules per file:
    ...
    if not paths.home_defaults.is_dir():
        ...
        return
    _walk_and_copy(paths.home_defaults, paths.mops_home)
    ...
    if paths.mops_home.parent.name != "agents":
...
def _smart_merge_config(paths: MopsPaths) -> None:
...
def _migrate_tasks_to_cron_tasks(paths: MopsPaths) -> None:
...
def _clean_orphan_symlinks(paths: MopsPaths) -> None:
...
- Your filesystem is isolated. `/mops` is the mounted host directory `~/.mops`.
