"""Cross-platform skill directory sync between mops workspace and CLI tools.

Provides multi-way symlink synchronization so skills installed via Claude Code,
Codex CLI, Gemini CLI, or the mops workspace are visible to all agents.

Includes bundled-skill linking (package -> workspace), sync-time external-symlink
protection, and cleanup of mops-created links on shutdown.

When Docker sandboxing is active, symlinks are replaced with directory copies
(marked with ``.mops_managed``) because absolute host paths do not resolve
...
def _resolve_canonical(
    ...
    *registries: dict[str, Path],
    ...
    Priority follows argument order (typically mops > claude > codex > gemini).
...
def _is_managed_copy(path: Path) -> bool:
    """Return ``True`` if *path* is a mops-managed copy (has marker file)."""
...
def _ensure_copy(dest: Path, source: Path) -> bool:
    """Copy *source* directory to *dest* with a ``.mops_managed`` marker.
...
def sync_skills(paths: MopsPaths, *, docker_active: bool = False) -> None:
    """Multi-way skill directory sync: mops workspace <-> CLI skill dirs.

    Syncs between mops workspace, ~/.claude/skills, ~/.codex/skills,
    and ~/.gemini/skills.
    ...
    all_dirs: dict[str, Path] = {"mops": paths.skills_dir, **cli_dirs}
    ...
    # Priority order: mops > claude > codex > gemini
    priority = ("mops", "claude", "codex", "gemini")
...
def _iter_bundled_entries(paths: MopsPaths) -> list[tuple[Path, Path]]:
...
def sync_bundled_skills(paths: MopsPaths, *, docker_active: bool = False) -> None:
    """Sync bundled skills from the package into the mops workspace.

    Creates symlinks (or copies when *docker_active*) from
    ``~/.mops/workspace/skills/<name>`` to the package's
    ``_home_defaults/workspace/skills/<name>`` so bundled skills
    stay up-to-date with the installed mops version.
...
def cleanup_mops_links(paths: MopsPaths) -> int:
    """Remove symlinks created by mops in CLI skill directories.

    Only removes symlinks whose resolved target is under the mops workspace
    ...
    for cli_dir in _cli_skill_dirs().values():
        ...
        for entry in cli_dir.iterdir():
            ...
            if any(_is_under(resolved, root) for root in managed_roots):
                entry.unlink()
                removed += 1
                logger.info("Removed mops skill link: %s", entry)

    if removed:
        logger.info("Cleaned up %d mops skill link(s) from CLI directories", removed)
    return removed
