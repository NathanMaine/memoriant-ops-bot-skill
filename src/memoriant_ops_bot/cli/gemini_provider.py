...
from memoriant_ops_bot.cli.auth import gemini_api_key_mode_selected
from memoriant_ops_bot.cli.base import (
    BaseCLI,
    CLIConfig,
...
from memoriant_ops_bot.cli.stream_events import ResultEvent, StreamEvent, SystemInitEvent
from memoriant_ops_bot.cli.types import CLIResponse
from memoriant_ops_bot.config import NULLISH_TEXT_VALUES
from memoriant_ops_bot.infra.platform import CREATION_FLAGS as _CREATION_FLAGS
from memoriant_ops_bot.infra.process_tree import force_kill_process_tree
from memoriant_ops_bot.workspace.paths import resolve_paths

if TYPE_CHECKING:
    from memoriant_ops_bot.cli.process_registry import ProcessRegistry, TrackedProcess
    from memoriant_ops_bot.cli.timeout_controller import TimeoutController

logger = logging.getLogger(__name__)

_DEFAULT_TIMEOUT = 300.0

# Must match ``_MOPS_MOUNT`` in ``memoriant_ops_bot.infra.docker``.
_CONTAINER_MOPS = "/mops"
...
class GeminiCLI(BaseCLI):
    ...
    def _inject_config_gemini_api_key(self, env: dict[str, str]) -> None:
        """Inject GEMINI_API_KEY from mops config when API-key auth mode is active."""
        ...
        logger.debug("Injected GEMINI_API_KEY from mops config for Gemini API key mode")
    ...
    def _create_system_prompt_path(self) -> str | None:
        ...
        In Docker mode the file is written to ``~/.mops/tmp/`` which is
        ...
        if self._config.docker_container:
            tmp_dir = resolve_paths().mops_home / "tmp"
    ...
    def _host_to_container_path(host_path: str) -> str | None:
        """Translate a host path under ``~/.mops/`` to its container mount."""
        prefix = str(resolve_paths().mops_home)
        if host_path.startswith(prefix):
            return _CONTAINER_MOPS + host_path[len(prefix) :].replace("\\", "/")
        return None
