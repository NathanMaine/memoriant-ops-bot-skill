...
from memoriant_ops_bot.cli.stream_events import ResultEvent, StreamEvent
...
def _build_subprocess_env(config: CLIConfig) -> dict[str, str] | None:
    ...
    agent identification variables.  User secrets from ``~/.mops/.env`` are
    merged in without overriding existing variables.
    """
    import os
    from pathlib import Path

    from memoriant_ops_bot.infra.env_secrets import load_env_secrets

    env = os.environ.copy()

    # Merge user secrets (low priority — never override existing vars).
    working_dir = Path(config.working_dir)
    mops_home = working_dir.parent if working_dir.name == "workspace" else working_dir
    env_file = mops_home / ".env"
    for key, value in load_env_secrets(env_file).items():
        if key not in env:
            env[key] = value

    env["MOPS_AGENT_NAME"] = config.agent_name
    env["MOPS_AGENT_ROLE"] = "main" if config.agent_name == "main" else "sub"
    env["MOPS_INTERAGENT_PORT"] = str(config.interagent_port)
    if config.chat_id:
        env["MOPS_CHAT_ID"] = str(config.chat_id)
    if config.topic_id:
        env["MOPS_TOPIC_ID"] = str(config.topic_id)
    env["MOPS_TRANSPORT"] = config.transport
    working_dir = Path(config.working_dir)
    mops_home = working_dir.parent if working_dir.name == "workspace" else working_dir
    env["MOPS_HOME"] = str(mops_home)
    # Shared knowledge is always at the main agent's home level.
    # For main: mops_home itself. For sub-agents: ../../ from agents/<name>/.
    if config.agent_name == "main":
        env["MOPS_SHARED_MEMORY_PATH"] = str(mops_home / "SHAREDMEMORY.md")
    else:
        # Sub-agent home is <main_home>/agents/<name>/
        main_home = mops_home.parent.parent
        env["MOPS_SHARED_MEMORY_PATH"] = str(main_home / "SHAREDMEMORY.md")
    return env
