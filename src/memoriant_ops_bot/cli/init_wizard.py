...
def _build_extras_table(console: Console) -> None:
    ...
    from memoriant_ops_bot.infra.docker_extras import DOCKER_EXTRAS_BY_ID, extras_for_display
...
def _offer_service_install(console: Console) -> bool:
    """Ask whether to install mops as a background service."""
    from memoriant_ops_bot.infra.service import is_service_available
...
def run_onboarding() -> bool:
    ...
    console.print(
        Panel(
            ...
            + "\n\n"
            + t_rich("wizard.complete.home", path=paths.mops_home)
            + "\n"
    ...
    if run_as_service:
        from memoriant_ops_bot.infra.service import install_service
...
def run_smart_reset(mops_home: Path) -> None:
    """Read existing config, handle Docker cleanup, and delete workspace."""
    console = Console()
    console.print()

    config_path = mops_home / "config" / "config.json"
    ...
    if config_path.exists():
        try:
            ...
            if isinstance(docker, dict) and docker.get("enabled"):
                docker_container = str(docker.get("container_name", "mops-sandbox"))
                docker_image = str(docker.get("image_name", "mops-sandbox"))
        except (json.JSONDecodeError, OSError):
            pass
    ...
    console.print(
        Panel(
            t_rich("wizard.reset.body", home=mops_home),
    ...
    if mops_home.exists():
        console.print(t_rich("wizard.reset.confirm.warning", home=mops_home) + "\n")
    else:
