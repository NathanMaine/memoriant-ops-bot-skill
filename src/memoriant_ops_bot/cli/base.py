...
_CONTAINER_MOPS_MOUNT = "/mops"


def _to_container_path(host_path: Path, main_home: Path) -> str:
    """Map a host path under *main_home* to its container equivalent.

    The Docker container mounts the root mops home at ``/mops``.
    """
    rel = host_path.relative_to(main_home)
    if str(rel) == ".":
        return _CONTAINER_MOPS_MOUNT
    return f"{_CONTAINER_MOPS_MOUNT}/{rel.as_posix()}"
...
def docker_wrap(
    ...
    *extra_env* vars are injected as ``-e`` flags into ``docker exec``
    ...
    if config.docker_container:
        ...
        # Resolve root mops home for host → container path mapping.
        ...
        container_home = _to_container_path(mops_home, main_home)
        ...
        env_flags: list[str] = [
            "-e",
            f"MOPS_CHAT_ID={config.chat_id}",
            "-e",
            f"MOPS_TRANSPORT={config.transport}",
            "-e",
            f"MOPS_AGENT_NAME={config.agent_name}",
            "-e",
            f"MOPS_INTERAGENT_PORT={config.interagent_port}",
            "-e",
            f"MOPS_HOME={container_home}",
            "-e",
            f"MOPS_SHARED_MEMORY_PATH={container_shared}",
            "-e",
            "MOPS_INTERAGENT_HOST=host.docker.internal",
        ]
        if config.topic_id:
            env_flags += ["-e", f"MOPS_TOPIC_ID={config.topic_id}"]
        if extra_env:
