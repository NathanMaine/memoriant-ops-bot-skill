...
Memoriant Ops Bot is a multi-transport chat orchestrator for the official provider CLIs (`claude`, `codex`, `gemini`).
It runs Telegram and/or Matrix, can expose an optional direct WebSocket API, keeps state under `~/.mops`, and supervises the main agent plus optional sub-agents in one asyncio process.
...
| `workspace/` | `~/.mops` path model, seeding, rule deployment/sync, skill sync |
...
- `MopsPaths` in `workspace/paths.py` is the single source of truth for runtime paths.
...
- Skill sync spans `~/.mops/workspace/skills`, `~/.claude/skills`, `~/.codex/skills`, `~/.gemini/skills`:
  ...
  - Docker mode: managed copies (`.mops_managed`)
...
- `mops`, `mops onboarding`, `mops reset`
- `mops status`, `mops stop`, `mops restart`, `mops upgrade`, `mops uninstall`
...
- `mops agents add` interactively scaffolds Telegram sub-agents only
