...
from memoriant_ops_bot.multiagent.supervisor import (
...
class TestStartupFailures:
    ...
    async def test_internal_api_start_failure_is_propagated(
        ...
        with (
            patch(
                "memoriant_ops_bot.multiagent.internal_api.InternalAgentAPI.start",
...
class TestStartSubAgent:
    ...
    async def test_handles_creation_failure(self, supervisor: AgentSupervisor) -> None:
        ...
        with patch(
            "memoriant_ops_bot.multiagent.supervisor.AgentStack.create",
...
class TestStopAll:
    ...
    async def test_cancellation_unblocks_start(self, supervisor: AgentSupervisor) -> None:
        ...
        with (
            ...
            patch(
                "memoriant_ops_bot.multiagent.shared_knowledge.SharedKnowledgeSync",
            ) as mock_sks_cls,
        ):
            mock_sks = MagicMock()
