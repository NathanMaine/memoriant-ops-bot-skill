...
from memoriant_ops_bot.cli.param_resolver import TaskOverrides, resolve_cli_config
from memoriant_ops_bot.config import AgentConfig
from memoriant_ops_bot.cron.execution import build_cmd
...
def mock_paths(tmp_path: Path) -> MopsPaths:
    """Create mock MopsPaths for testing."""
    home = tmp_path / "mops_home"
    fw = tmp_path / "fw"
    paths = MopsPaths(
        mops_home=home,
...
async def test_cron_task_model_override(
    ...
    # Build command
    with patch("memoriant_ops_bot.cron.execution.which", return_value="/usr/bin/claude"):
...
async def test_cron_task_cli_parameters(
    ...
    # Build command
    with patch("memoriant_ops_bot.cron.execution.which", return_value="/usr/bin/codex"):
        result = build_cmd(exec_config, "Test prompt")
