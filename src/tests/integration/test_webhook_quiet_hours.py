...
from memoriant_ops_bot.config import AgentConfig, HeartbeatConfig, WebhookConfig
from memoriant_ops_bot.webhook.manager import WebhookManager
from memoriant_ops_bot.webhook.models import WebhookEntry
...
def _make_manager(paths: MopsPaths) -> WebhookManager:
...
def _add_hook(mgr: WebhookManager, paths: MopsPaths, **overrides: Any) -> WebhookEntry:
    defaults: dict[str, Any] = {
        "id": "test-hook",
        "title": "Test Hook",
        "description": "Testing",
...
async def test_webhook_ignores_heartbeat_quiet_hours(tmp_path: Path) -> None:
    ...
    with patch("memoriant_ops_bot.cron.execution.build_cmd", return_value=None):
