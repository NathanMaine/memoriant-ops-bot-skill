...
class TestSendRich:
    ...
    async def test_plain_text_sent_as_html(self) -> None:
        from memoriant_ops_bot.messenger.telegram.sender import send_rich
...
class TestSendFile:
    ...
    async def test_image_sent_as_photo(self, tmp_path: Path) -> None:
        from memoriant_ops_bot.messenger.telegram.sender import send_file
...
class TestExtractFilePaths:
    ...
    def test_single_file(self) -> None:
        from memoriant_ops_bot.messenger.telegram.sender import extract_file_paths
...
class TestSendFilesFromText:
    ...
    async def test_sends_files_from_tags(self, tmp_path: Path) -> None:
        from memoriant_ops_bot.messenger.telegram.sender import send_files_from_text
    ...
    async def test_windows_slash_drive_tag_normalized_before_send(self) -> None:
        ...
        with (
            patch("memoriant_ops_bot.files.tags.is_windows", return_value=True),
            patch(
                "memoriant_ops_bot.messenger.telegram.sender.send_file", new_callable=AsyncMock
            ) as mock_send_file,
        ):
