...
@pytest.fixture
def orch_ia(workspace: tuple[MopsPaths, AgentConfig]) -> Orchestrator:
...
class TestInteragentChatId:
    ...
    def test_returns_zero_when_no_users(self, workspace: tuple[MopsPaths, AgentConfig]) -> None:
        paths, config = workspace
...
class TestHandleAsyncInteragentResult:
    ...
    async def test_resumes_current_active_session(self, orch_ia: Orchestrator) -> None:
        ...
        from memoriant_ops_bot.session import SessionData
