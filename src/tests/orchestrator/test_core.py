...
from memoriant_ops_bot.cli.types import AgentResponse
from memoriant_ops_bot.config import AgentConfig
from memoriant_ops_bot.errors import CLIError, CronError, SessionError, StreamError, WorkspaceError
...
async def test_create_with_authenticated_provider(
    ...
    with (
        patch(
            "memoriant_ops_bot.orchestrator.lifecycle.resolve_paths",
            return_value=paths,
        ),
        patch(
            "memoriant_ops_bot.cli.auth.check_all_auth",
            return_value={"claude": claude_auth, "codex": codex_auth},
        ),
        patch(
            "memoriant_ops_bot.orchestrator.observers.watch_rule_files",
...
async def test_handle_heartbeat_delegates_to_flow(orch: Orchestrator) -> None:
    with patch(
        "memoriant_ops_bot.orchestrator.core.heartbeat_flow",
        new_callable=AsyncMock,
