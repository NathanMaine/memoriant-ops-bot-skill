...
class TestPathFromFileTag:
    def test_windows_path_slash_drive_form(self) -> None:
        with patch("memoriant_ops_bot.files.tags.is_windows", return_value=True):
    ...
    def test_posix_path_unchanged(self) -> None:
        with patch("memoriant_ops_bot.files.tags.is_windows", return_value=False):
    ...
    def test_docker_path_translated(self) -> None:
        with (
            patch.dict("os.environ", {"MOPS_HOME": "/home/user/.mops"}),
            patch("memoriant_ops_bot.files.tags.is_windows", return_value=False),
        ):
            path = path_from_file_tag("/mops/workspace/output_to_user/img.png")
        assert path == Path("/home/user/.mops/workspace/output_to_user/img.png")
    ...
    def test_docker_path_root(self) -> None:
        with (
            ...
            path = path_from_file_tag("/mops/sessions.json")
        assert path == Path("/home/user/.mops/sessions.json")

    def test_non_docker_path_not_translated(self) -> None:
        with patch("memoriant_ops_bot.files.tags.is_windows", return_value=False):
            path = path_from_file_tag("/home/user/.mops/workspace/file.txt")
        assert path == Path("/home/user/.mops/workspace/file.txt")
