...
from memoriant_ops_bot.background.models import BackgroundResult, BackgroundSubmit
from memoriant_ops_bot.background.observer import MAX_TASKS_PER_CHAT, BackgroundObserver
from memoriant_ops_bot.cli.param_resolver import TaskExecutionConfig
from memoriant_ops_bot.cron.execution import OneShotExecutionResult
from memoriant_ops_bot.infra.task_runner import TaskResult
...
def _make_observer(paths: MopsPaths, timeout: float = 300.0) -> BackgroundObserver:
...
@pytest.fixture
async def observer(paths: MopsPaths) -> AsyncIterator[BackgroundObserver]:
...
class TestSubmit:
    async def test_returns_task_id(self, observer: BackgroundObserver) -> None:
        ...
        with patch(
            "memoriant_ops_bot.background.observer.run_oneshot_task",
...
class TestExecution:
    async def test_success_delivers_result(self, observer: BackgroundObserver) -> None:
        ...
        with patch("memoriant_ops_bot.background.observer.run_oneshot_task", return_value=result):
