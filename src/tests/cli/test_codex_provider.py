...
from memoriant_ops_bot.cli.codex_provider import CodexCLI, _codex_final_result, _log_cmd
from memoriant_ops_bot.cli.executor import SubprocessResult
...
class TestInit:
    def test_find_cli_raises_when_not_on_path(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("memoriant_ops_bot.cli.codex_provider.which", lambda _: None)
...
class TestSend:
    async def test_send_success(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with patch("memoriant_ops_bot.cli.executor.asyncio") as mock_asyncio:
...
class TestLogCmd:
    def test_short_values_not_truncated(self, caplog: pytest.LogCaptureFixture) -> None:
        with caplog.at_level(logging.INFO, logger="memoriant_ops_bot.cli.codex_provider"):
...
class TestDockerIntegration:
    ...
    async def test_send_with_docker_container_keeps_stdin_open_on_windows(
        ...
        monkeypatch.setattr("memoriant_ops_bot.cli.codex_provider._IS_WINDOWS", True)
        cli = CodexCLI(
            CLIConfig(
                provider="codex",
                model="gpt-5.2-codex",
...
class TestEdgeCases:
    ...
    def test_parse_output_error_logged(self, caplog: pytest.LogCaptureFixture) -> None:
        with caplog.at_level(logging.ERROR, logger="memoriant_ops_bot.cli.codex_provider"):
