...
from memoriant_ops_bot.cli.param_resolver import (
    TaskOverrides,
    resolve_cli_config,
)
from memoriant_ops_bot.config import AgentConfig, reset_gemini_models, set_gemini_models
from memoriant_ops_bot.errors import MopsError
...
def test_resolve_global_only(base_config: AgentConfig, codex_cache: CodexModelCache) -> None:
    ...
    assert result.working_dir == "~/mops"
...
def test_resolve_invalid_claude_model(
    ...
    with pytest.raises(MopsError, match="Invalid Claude model"):
...
def test_resolve_invalid_codex_model(
    ...
    with pytest.raises(MopsError, match="Invalid Codex model"):
        resolve_cli_config(base_config, codex_cache, task_overrides=overrides)
...
def test_resolve_gemini_invalid_against_discovered_models(
    ...
    with pytest.raises(MopsError, match="Invalid Gemini model"):
        resolve_cli_config(base_config, codex_cache, task_overrides=overrides)
