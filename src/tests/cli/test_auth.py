...
def test_check_gemini_auth_mops_config_key(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    mops_config = tmp_path / ".mops" / "config" / "config.json"
    mops_config.parent.mkdir(parents=True)
    mops_config.write_text('{"gemini_api_key":"from-mops-config"}')

    result = check_gemini_auth()

    assert result.status == AuthStatus.AUTHENTICATED
    assert result.auth_file == mops_config


def test_check_gemini_auth_mops_config_null_string_ignored(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    import memoriant_ops_bot.cli.auth as _auth_mod
    ...
    mops_config = tmp_path / ".mops" / "config" / "config.json"
    mops_config.parent.mkdir(parents=True)
    mops_config.write_text('{"gemini_api_key":"null"}')
