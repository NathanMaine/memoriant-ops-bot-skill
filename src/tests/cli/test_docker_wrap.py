...
from memoriant_ops_bot.cli.base import CLIConfig, docker_wrap
...
def test_docker_wrap_with_container() -> None:
    ...
    assert result_cmd == [
        ...
        "MOPS_TRANSPORT=tg",
        "-e",
        "MOPS_AGENT_NAME=main",
        "-e",
        "MOPS_INTERAGENT_PORT=8799",
        "-e",
        "MOPS_HOME=/mops",
        "-e",
        "MOPS_SHARED_MEMORY_PATH=/mops/SHAREDMEMORY.md",
        "-e",
        "MOPS_INTERAGENT_HOST=host.docker.internal",
        "my-sandbox",
        "claude",
        "-p",
        "hello",
...
def test_docker_wrap_injects_chat_id() -> None:
    ...
    result_cmd, _ = docker_wrap(cmd, cfg)
    assert "MOPS_CHAT_ID=999" in result_cmd
...
def test_docker_wrap_sub_agent_container_paths() -> None:
    """Sub-agent working_dir maps to /mops/agents/<name>/workspace inside container."""
    cmd = ["claude", "-p", "hi"]
    cfg = CLIConfig(
        docker_container="sandbox",
        chat_id=1,
        working_dir="/home/user/.mops/agents/test/workspace",
        agent_name="test",
    )
    result_cmd, cwd = docker_wrap(cmd, cfg)
    assert cwd is None
    ...
    assert result_cmd[w_idx + 1] == "/mops/agents/test/workspace"
    # MOPS_HOME is the sub-agent home inside the container
    assert "MOPS_HOME=/mops/agents/test" in result_cmd
    # Shared memory is at the root
    assert "MOPS_SHARED_MEMORY_PATH=/mops/SHAREDMEMORY.md" in result_cmd


def test_docker_wrap_main_agent_container_paths() -> None:
    """Main agent working_dir maps to /mops/workspace inside container."""
    cmd = ["claude", "-p", "hi"]
    cfg = CLIConfig(
        docker_container="sandbox",
        chat_id=1,
        working_dir="/home/user/.mops/workspace",
        agent_name="main",
    )
    result_cmd, _ = docker_wrap(cmd, cfg)
    w_idx = result_cmd.index("-w")
    assert result_cmd[w_idx + 1] == "/mops/workspace"
    assert "MOPS_HOME=/mops" in result_cmd
    assert "MOPS_SHARED_MEMORY_PATH=/mops/SHAREDMEMORY.md" in result_cmd
...
def test_docker_wrap_sub_agent_windows_paths_are_posix() -> None:
    ...
    cfg = CLIConfig(
        docker_container="sandbox",
        chat_id=1,
        working_dir=PureWindowsPath(r"C:\Users\me\.mops\agents\seismic-bot\workspace"),
        agent_name="seismic-bot",
    )
    result_cmd, _ = docker_wrap(cmd, cfg, interactive=True)
    w_idx = result_cmd.index("-w")
    assert result_cmd[w_idx + 1] == "/mops/agents/seismic-bot/workspace"
    assert "MOPS_HOME=/mops/agents/seismic-bot" in result_cmd
