...
def _setup_home_defaults(fw_root: Path) -> None:
    ...
    Mirrors the repo ``workspace/`` structure (1:1 copy of ~/.mops/).
    """
    ws = fw_root / "workspace"

    # Top-level CLAUDE.md (mops home)
    ws.mkdir(parents=True)
    (ws / "CLAUDE.md").write_text("# Mops Home CLAUDE.md")
...
def test_mops_home_claude_md_overwritten(tmp_path: Path) -> None:
    """The mops_home/CLAUDE.md is Zone 2 (always overwritten)."""
    paths = _make_paths(tmp_path)
    init_workspace(paths)

    home_claude = paths.mops_home / "CLAUDE.md"
    assert home_claude.exists()
    assert home_claude.read_text() == "# Mops Home CLAUDE.md"
...
def test_inject_docker_notice(tmp_path: Path) -> None:
    paths = _make_paths(tmp_path)
    init_workspace(paths)
    inject_runtime_environment(paths, docker_container="mops-sandbox")
    content = (paths.workspace / "CLAUDE.md").read_text()
    assert "DOCKER CONTAINER" in content
    assert "mops-sandbox" in content
