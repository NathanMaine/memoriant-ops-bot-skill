...
def test_canonical_mops_priority(tmp_path: Path) -> None:
    mops = {"sk": tmp_path / "mops" / "sk"}
    claude = {"sk": tmp_path / "claude" / "sk"}
    codex = {"sk": tmp_path / "codex" / "sk"}
    for d in (mops["sk"], claude["sk"], codex["sk"]):
        d.mkdir(parents=True)
    result = _resolve_canonical("sk", mops, claude, codex)
    assert result == mops["sk"]
...
def test_sync_claude_to_mops(tmp_path: Path) -> None:
    paths, claude_skills, _ = _setup_three_dirs(tmp_path)
    _make_skill(claude_skills, "from-claude")
    with patch("memoriant_ops_bot.workspace.skill_sync._cli_skill_dirs") as mock:
...
def test_sync_codex_to_mops(tmp_path: Path) -> None:
...
def test_sync_mops_to_both(tmp_path: Path) -> None:
    paths, claude_skills, codex_skills = _setup_three_dirs(tmp_path)
    claude_skills.mkdir(parents=True, exist_ok=True)
    codex_skills.mkdir(parents=True, exist_ok=True)
    _make_skill(paths.skills_dir, "from-mops")
    with patch("memoriant_ops_bot.workspace.skill_sync._cli_skill_dirs") as mock:
    ...
    for d in (claude_skills, codex_skills):
        link = d / "from-mops"
        assert link.is_symlink()
        assert link.resolve() == (paths.skills_dir / "from-mops").resolve()


def test_sync_gemini_to_mops(tmp_path: Path) -> None:
...
def test_sync_mops_to_all_three(tmp_path: Path) -> None:
    paths, claude_skills, codex_skills = _setup_three_dirs(tmp_path)
    gemini_home = tmp_path / "fake_home" / ".gemini"
    gemini_home.mkdir(parents=True)
    gemini_skills = gemini_home / "skills"
...
def test_sync_external_symlink(tmp_path: Path) -> None:
    ...
    with patch("memoriant_ops_bot.workspace.skill_sync._cli_skill_dirs") as mock:
        mock.return_value = {"claude": claude_skills, "codex": codex_skills}
        sync_skills(paths)
    mops_link = paths.skills_dir / "ext-skill"
    assert mops_link.is_symlink()
    assert mops_link.resolve() == external_real.resolve()
...
# ---------------------------------------------------------------------------
# Group 7: MopsPaths.skills_dir property
# ---------------------------------------------------------------------------
...
def test_sync_preserves_external_symlink(tmp_path: Path) -> None:
    ...
    # Mops also has a skill with the same name
    _make_skill(paths.skills_dir, "my-skill")
...
def test_sync_replaces_internal_symlink(tmp_path: Path) -> None:
    """Symlinks pointing inside sync dirs (mops-managed) can be updated."""
    paths, claude_skills, codex_skills = _setup_three_dirs(tmp_path)
    claude_skills.mkdir(parents=True, exist_ok=True)
    codex_skills.mkdir(parents=True, exist_ok=True)
    ...
    # Mops now has a real skill with the same name (higher priority)
    ...
    # Claude's symlink should now point to mops (higher priority canonical)
...
# ---------------------------------------------------------------------------
# Group 12: cleanup_mops_links
# ---------------------------------------------------------------------------


def test_cleanup_removes_mops_links(tmp_path: Path) -> None:
    paths, claude_skills, codex_skills = _setup_three_dirs(tmp_path)
    claude_skills.mkdir(parents=True, exist_ok=True)
    codex_skills.mkdir(parents=True, exist_ok=True)

    # Simulate mops-created symlinks (target under mops skills dir)
    _make_skill(paths.skills_dir, "from-mops")
    (claude_skills / "from-mops").symlink_to(paths.skills_dir / "from-mops")
    (codex_skills / "from-mops").symlink_to(paths.skills_dir / "from-mops")

    with patch("memoriant_ops_bot.workspace.skill_sync._cli_skill_dirs") as mock:
        mock.return_value = {"claude": claude_skills, "codex": codex_skills}
        removed = cleanup_mops_links(paths)

    assert removed == 2
    assert not (claude_skills / "from-mops").exists()
    assert not (codex_skills / "from-mops").exists()
