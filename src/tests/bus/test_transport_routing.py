...
from memoriant_ops_bot.bus.envelope import DeliveryMode, Envelope, Origin
...
class TestCascadingFallback:
    ...
    async def test_fallback_logs_warning(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        env = _env(transport="mx", delivery=DeliveryMode.UNICAST)
        with caplog.at_level("WARNING", logger="memoriant_ops_bot.bus.bus"):
            await bus.submit(env)
    ...
    async def test_fallback_delivery_failure_is_logged(
        ...
        env = _env(transport="mx", delivery=DeliveryMode.UNICAST)
        with caplog.at_level("ERROR", logger="memoriant_ops_bot.bus.bus"):
            await bus.submit(env)
...
class TestAdapterTransportParam:
    def test_from_cron_result_includes_transport(self) -> None:
        ...
        from memoriant_ops_bot.bus.adapters import from_cron_result
    ...
    def test_from_heartbeat_includes_transport(self) -> None:
        ...
        from memoriant_ops_bot.bus.adapters import from_heartbeat
...
class TestCronJobTransport:
    def test_cron_job_has_transport_field(self) -> None:
        ...
        from memoriant_ops_bot.cron.manager import CronJob
...
# -- Executor MOPS_TRANSPORT env var --


class TestExecutorTransportEnv:
    def test_build_subprocess_env_includes_transport(self) -> None:
        """_build_subprocess_env sets MOPS_TRANSPORT from CLIConfig."""
        from memoriant_ops_bot.cli.base import CLIConfig
        from memoriant_ops_bot.cli.executor import _build_subprocess_env
        ...
        assert env["MOPS_TRANSPORT"] == "mx"

    def test_build_subprocess_env_default_transport(self) -> None:
        """_build_subprocess_env defaults MOPS_TRANSPORT to 'tg'."""
        from memoriant_ops_bot.cli.base import CLIConfig
        from memoriant_ops_bot.cli.executor import _build_subprocess_env
