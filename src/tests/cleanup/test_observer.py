...
from memoriant_ops_bot.cleanup.observer import CleanupObserver, _delete_old_files
from memoriant_ops_bot.config import AgentConfig, CleanupConfig
...
def _make_paths(tmp_path: Path) -> MopsPaths:
    return MopsPaths(mops_home=tmp_path)
...
async def test_maybe_run_skips_wrong_hour(tmp_path: Path) -> None:
    ...
    with patch("memoriant_ops_bot.cleanup.observer.datetime") as mock_dt:
