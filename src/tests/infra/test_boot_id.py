...
from memoriant_ops_bot.infra.boot_id import get_boot_id
...
class TestGetBootIdLinux:
    ...
    def test_reads_proc_boot_id(self, mock_sys: MagicMock, tmp_path: Path) -> None:
        ...
        with patch("memoriant_ops_bot.infra.boot_id._LINUX_BOOT_ID_PATH", boot_file):
            result = get_boot_id()
...
class TestGetBootIdMacOS:
    @patch("memoriant_ops_bot.infra.boot_id.sys")
    @patch("memoriant_ops_bot.infra.boot_id.subprocess")
...
class TestGetBootIdWindows:
    ...
    def test_uses_uptime(self, mock_sys: MagicMock) -> None:
        ...
        with patch("memoriant_ops_bot.infra.boot_id._windows_boot_id", return_value="win-12345"):
            result = get_boot_id()
    ...
    def test_windows_failure_returns_empty(self, mock_sys: MagicMock) -> None:
        mock_sys.platform = "win32"
        with patch("memoriant_ops_bot.infra.boot_id._windows_boot_id", return_value=""):
            result = get_boot_id()
        assert result == ""
