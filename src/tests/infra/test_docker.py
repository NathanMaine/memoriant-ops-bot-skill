...
from memoriant_ops_bot.config import DockerConfig
from memoriant_ops_bot.workspace.paths import MopsPaths


@pytest.fixture
def docker_paths(tmp_path: Path) -> MopsPaths:
    home = tmp_path / ".mops"
    home.mkdir()
    ws = home / "workspace"
    ws.mkdir()
...
class TestDockerManager:
    ...
    async def test_setup_returns_none_when_docker_not_found(
        self, docker_config: DockerConfig, docker_paths: MopsPaths
    ) -> None:
        from memoriant_ops_bot.infra.docker import DockerManager

        mgr = DockerManager(docker_config, docker_paths)
        with patch("memoriant_ops_bot.infra.docker.which", return_value=None):
    ...
    async def test_setup_returns_none_when_daemon_unavailable(
        ...
        mgr = DockerManager(docker_config, docker_paths)
        with (
            patch("memoriant_ops_bot.infra.docker.which", return_value="/usr/bin/docker"),
    ...
    async def test_mounts_full_mops_home(
        self, docker_config: DockerConfig, docker_paths: MopsPaths
    ) -> None:
        """Verify run command mounts entire ~/.mops, not just workspace."""
        from memoriant_ops_bot.infra.docker import DockerManager
        ...
        # Full mops_home mounted at /mops
        assert f"{docker_paths.mops_home}:/mops" in run_str
        # Working dir is /mops/workspace
        assert "-w /mops/workspace" in run_str
        # MOPS_HOME env var set inside container
        assert "MOPS_HOME=/mops" in run_str
    ...
    async def test_uid_mapping_on_linux(
        ...
        with (
            patch("memoriant_ops_bot.infra.docker.which", return_value="/usr/bin/docker"),
            patch.object(mgr, "_exec", side_effect=mock_exec),
            patch("memoriant_ops_bot.infra.docker._needs_uid_mapping", return_value=True),
    ...
    async def test_no_uid_mapping_on_macos(
        ...
        with (
            patch("memoriant_ops_bot.infra.docker.which", return_value="/usr/bin/docker"),
            patch.object(mgr, "_exec", side_effect=mock_exec),
            patch("memoriant_ops_bot.infra.docker._needs_uid_mapping", return_value=False),
        ):
            await mgr.setup()
    ...
    async def test_sub_agent_mounts_root_mops_home(
        self, docker_config: DockerConfig, tmp_path: Path
    ) -> None:
        """Sub-agent container mounts ~/.mops (root), not ~/.mops/agents/test."""
        from memoriant_ops_bot.infra.docker import DockerManager

        root_home = tmp_path / ".mops"
        ...
        fw = tmp_path / "framework"
        fw.mkdir()
        paths = MopsPaths(mops_home=agent_home, home_defaults=fw / "workspace", framework_root=fw)
        ...
        assert f"{root_home}:/mops" in run_str
        assert f"{agent_home}:/mops" not in run_str
