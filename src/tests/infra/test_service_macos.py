...
from memoriant_ops_bot.infra.service_macos import (
    _LABEL,
...
class TestGeneratePlistData:
    def test_contains_binary_path(self) -> None:
        data = _generate_plist_data("/usr/local/bin/mops")
        assert data["ProgramArguments"] == ["/usr/local/bin/mops"]
    ...
    def test_path_includes_all_nvm_bins(self, tmp_path: Path) -> None:
        ...
        with patch("memoriant_ops_bot.infra.service_macos.Path.home", return_value=tmp_path):
            data = _generate_plist_data("mops")
...
class TestIsServiceInstalled:
    @patch("memoriant_ops_bot.infra.service_macos._plist_path")
...
class TestIsServiceRunning:
    @patch("memoriant_ops_bot.infra.service_macos.is_service_installed", return_value=False)
    def test_not_running_when_not_installed(self, _mock: MagicMock) -> None:
        assert is_service_running() is False

    @patch("memoriant_ops_bot.infra.service_macos._run_launchctl")
    @patch("memoriant_ops_bot.infra.service_macos.is_service_installed", return_value=True)
...
class TestInstallService:
    @patch("memoriant_ops_bot.infra.service_macos._run_launchctl")
    @patch("memoriant_ops_bot.infra.service_macos.is_service_installed", return_value=False)
    @patch("memoriant_ops_bot.infra.service_macos.is_service_available", return_value=True)
    @patch(
        "memoriant_ops_bot.infra.service_macos.find_mops_binary", return_value="/usr/local/bin/mops"
    )
    @patch("memoriant_ops_bot.infra.service_macos._plist_path")
    @patch("memoriant_ops_bot.infra.service_macos.resolve_paths")
    def test_install_success(
        ...
        plist_file = tmp_path / "dev.mops.plist"
    ...
    @patch("memoriant_ops_bot.infra.service_macos.is_service_available", return_value=True)
    @patch("memoriant_ops_bot.infra.service_macos.find_mops_binary", return_value=None)
...
class TestStopService:
    @patch("memoriant_ops_bot.infra.service_macos._run_launchctl")
    @patch("memoriant_ops_bot.infra.service_macos.is_service_running", return_value=True)
    ...
    @patch("memoriant_ops_bot.infra.service_macos.is_service_running", return_value=False)
...
class TestPrintServiceLogs:
    ...
    def test_shows_logs_from_file(
        ...
        logs_dir.mkdir()
        log_file = logs_dir / "mops_2026-02-22.log"
