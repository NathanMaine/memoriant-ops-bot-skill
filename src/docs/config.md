...
Runtime config file: `~/.mops/config/config.json`.

Seed source: `<repo>/config.example.json` (source checkout) or packaged fallback `mops_bot/_config_example.json` (installed mode).

## Config Creation

Primary path: `mops onboarding` (interactive wizard) writes `config.json` with user-provided values merged into `AgentConfig` defaults.

## Load & Merge Behavior

Config is merged in two places:

1. `mops_bot/__main__.py::load_config()`
...
2. `mops_bot/workspace/init.py::_smart_merge_config()`
...
- `mops api enable` writes the `api` block (including generated token) into `config.json`.

## External API Secrets (`~/.mops/.env`)
...
## `AgentConfig` (`mops_bot/config.py`)
...
| `mops_home` | `str` | `"~/.mops"` | Runtime home root |
...
| `user_id` | `str` | `""` | Bot user ID (e.g. `@mops:matrix.org`) |
...
| `store_path` | `str` | `"matrix_store"` | E2EE key store directory, relative to `mops_home` |

Notes:

- first successful login persists credentials to `~/.mops/<store_path>/credentials.json` (mode `0o600`), not back into `config.json`
...
- `~/.mops/cron_jobs.json` (`CronJob`)
- `~/.mops/webhooks.json` (`WebhookEntry`, `cron_task` mode)
...
| `image_name` | `str` | `"mops-sandbox"` | Docker image name |
| `container_name` | `str` | `"mops-sandbox"` | Docker container name |
...
`Orchestrator.create()` calls `DockerManager.setup()` when enabled. If setup fails, mops logs warning and falls back to host execution.
...
- updates are typically managed via `mops docker mount|unmount`
- changing mounts requires bot restart (or `mops docker rebuild`) to affect container run flags

### `extras`

Optional AI/ML packages installed into the Docker sandbox image at build time. Each entry is an ID from the extras registry (`mops_bot/infra/docker_extras.py`).
...
Managed via `mops docker extras-add|extras-remove` or during onboarding wizard. Changes require `mops docker rebuild` to take effect.
...
| `token` | `str` | `""` | Bearer/WebSocket auth token (generated by `mops api enable`, with runtime generation fallback on API start) |
...
- `docker`, `api`, `webhooks`
- `mops_home`, `log_level`, `gemini_api_key`, `timeouts`, `tasks`

Restart classification is computed from `AgentConfig` top-level schema fields.

## Model Resolution

`ModelRegistry` (`mops_bot/config.py`):
...
`resolve_user_timezone(configured)` in `mops_bot/config.py`:
...
Path: `~/.mops/config/codex_models.json`.
...
Path: `~/.mops/config/gemini_models.json`.
