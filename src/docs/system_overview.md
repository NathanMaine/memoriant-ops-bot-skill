...
Fastest end-to-end mental model for mops.
...
- optional sub-agent stacks from `~/.mops/agents.json`
...
Sub-agent home: `~/.mops/agents/<name>/` with its own config/workspace/session files.

## 8) Where to read code first

1. `mops_bot/__main__.py` (entrypoint + config/load/run)
2. `mops_bot/cli_commands/` (actual CLI subcommand logic)
3. `mops_bot/multiagent/supervisor.py` (always-on runtime wrapper)
4. `mops_bot/messenger/telegram/app.py` + `messenger/telegram/startup.py` (Telegram), `mops_bot/messenger/matrix/bot.py` (Matrix)
5. `mops_bot/orchestrator/core.py` + `orchestrator/lifecycle.py`
6. `mops_bot/bus/*` (unified delivery/injection)
7. `mops_bot/tasks/hub.py` + `tasks/registry.py`
8. `mops_bot/cli/service.py` and provider wrappers
...
- `mops`
- `mops status|stop|restart|upgrade|uninstall|onboarding|reset|help`
- `mops service ...`
- `mops docker ...` (includes `extras`, `extras-add`, `extras-remove` for optional AI/ML packages)
- `mops api ...`
- `mops agents ...` (`add` currently scaffolds Telegram sub-agents; Matrix sub-agents are added via `agents.json` or tool scripts)
- `mops install <extra>` (`matrix`, `api`)
...
- `mops service logs` follows `journalctl` on Linux and tails file logs on macOS/Windows
