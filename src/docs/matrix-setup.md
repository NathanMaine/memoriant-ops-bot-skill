...
mops's primary transport is Telegram. Matrix is an optional second transport you can add at any time — either as the only transport or running alongside Telegram.
...
# pip
pip install "memoriant-ops-bot[matrix]"
...
| User ID | `@my_mops_bot:matrix.org` |
...
```json
{
  ...
  "matrix": {
    ...
    "user_id": "@my_mops_bot:matrix.org",
...
On first start, mops logs in with the password and saves credentials to `~/.mops/matrix_store/credentials.json`. After that, the password is no longer needed — token-based auth is used automatically.
...
| `store_path` | no | Relative to mops_home. Default: `matrix_store` |
...
| Sub-agent setup | `mops agents add` (interactive) | Manual via `agents.json` |
...
- Run `mops install matrix` to install the dependency
...
- Delete `~/.mops/matrix_store/credentials.json` and restart — mops will re-login with password
