...
mops supports multiple messaging transports. The `transport` config
...
### `mops` entry (`mops_bot/__main__.py`)
...
1. resolve paths and set `MOPS_HOME` for main agent
...
- shared registry: `~/.mops/tasks.json`
- folders: `~/.mops/workspace/tasks/<task_id>/`
...
- task tools receive `MOPS_CHAT_ID` and optional `MOPS_TOPIC_ID`
...
Source: `mops_bot/_home_defaults/`.
