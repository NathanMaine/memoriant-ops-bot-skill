...
- persisted in `~/.mops/tasks.json`
- task folders in `~/.mops/workspace/tasks/<task_id>/`
...
- forum-topic tasks route back to the originating topic via `thread_id` / `MOPS_TOPIC_ID`

## Cron jobs

Cron jobs run in `~/.mops/workspace/cron_tasks/<task_folder>/`.
...
```text
~/.mops/workspace/cron_tasks/weather-report/
  CLAUDE.md
  AGENTS.md
  TASK_DESCRIPTION.md
