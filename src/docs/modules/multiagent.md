...
Multi-agent runtime: run multiple independent mops agents in one process.
...
- workspace and files under `~/.mops/agents/<name>/`
...
- central log file (`~/.mops/logs/agent.log`)
- shared knowledge source (`~/.mops/SHAREDMEMORY.md`)
...
`SharedKnowledgeSync` watches `~/.mops/SHAREDMEMORY.md` and mirrors content into each agent's `MAINMEMORY.md` block.
...
`mops agents list` fetches live health from internal API when main bot is running.

Important CLI nuance:

- `mops agents add <name>` currently prompts for Telegram token/user/group data only.
