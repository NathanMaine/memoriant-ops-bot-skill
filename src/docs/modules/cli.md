...
- `MOPS_CHAT_ID`
- `MOPS_TOPIC_ID` (when set)
- `MOPS_TRANSPORT` (active transport identifier, e.g. `"tg"`, `"mx"`)
...
- may inject `GEMINI_API_KEY` from mops config when Gemini settings indicate API-key mode and no env key is set
...
- Gemini:
  ...
  - optional fallback to `~/.mops/config/config.json` `gemini_api_key`

## Model caches

### Codex cache

- file: `~/.mops/config/codex_models.json`
...
- file: `~/.mops/config/gemini_models.json`
...
- container mode:
  ...
  - injects `MOPS_CHAT_ID`, optional `MOPS_TOPIC_ID`, `MOPS_TRANSPORT`, `MOPS_AGENT_NAME`, `MOPS_INTERAGENT_PORT`, `MOPS_HOME`, `MOPS_SHARED_MEMORY_PATH`, and `MOPS_INTERAGENT_HOST`,
  - merges user secrets from `~/.mops/.env` (never overrides existing vars),
