...
- `mops_bot/config_reload.py`: `ConfigReloader`, config diff/classification helpers

## Purpose

Avoid unnecessary restarts when safe `config.json` fields change.

`ConfigReloader` polls `~/.mops/config/config.json` every 5 seconds, validates with `AgentConfig`, diffs top-level schema fields, and:
...
- environment/core: `mops_home`, `log_level`, `gemini_api_key`, `update_check`
