...
Workspace/home-directory management for `~/.mops`.

## Files

- `paths.py`: immutable `MopsPaths` + `resolve_paths()`
...
- `mops_home`: `~/.mops` (default)
- `workspace`: `~/.mops/workspace`
- `config_path`: `~/.mops/config/config.json`
- `sessions_path`: `~/.mops/sessions.json`
- `named_sessions_path`: `~/.mops/named_sessions.json`
- `env_file`: `~/.mops/.env`
- `tasks_registry_path`: `~/.mops/tasks.json`
- `chat_activity_path`: `~/.mops/chat_activity.json`
- `startup_state_path`: `~/.mops/startup_state.json`
- `inflight_turns_path`: `~/.mops/inflight_turns.json`
- `cron_jobs_path`: `~/.mops/cron_jobs.json`
- `webhooks_path`: `~/.mops/webhooks.json`
- `logs_dir`: `~/.mops/logs`
- `cron_tasks_dir`: `~/.mops/workspace/cron_tasks`
- `tasks_dir`: `~/.mops/workspace/tasks`
- `api_files_dir`: `~/.mops/workspace/api_files`
- `skills_dir`: `~/.mops/workspace/skills`
...
- sub-agent homes do not create `logs/` by default; all agents write to the main home log file `~/.mops/logs/agent.log`.
...
- `~/.mops/workspace/skills`
- `~/.claude/skills`
- `~/.codex/skills`
- `~/.gemini/skills`

Default mode uses symlinks/junctions. Docker mode uses managed directory copies (`.mops_managed`) so paths resolve inside container namespace.
