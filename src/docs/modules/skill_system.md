...
Cross-tool skill sync between mops workspace and CLI skill homes.
...
- main: `~/.mops`
- sub-agent: `~/.mops/agents/<name>`

## Bundled skills

Bundled source: `mops_bot/_home_defaults/workspace/skills/`.
...
2. union names
3. pick canonical source by priority (`mops > claude > codex > gemini`)
...
`cleanup_mops_links(paths)` removes managed links under CLI skill dirs whose targets point to managed roots.
