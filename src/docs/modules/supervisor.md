...
- sub-agents are loaded from `~/.mops/agents.json`
...
3. if `tasks.enabled=true`: create shared `TaskHub` (`~/.mops/tasks.json` + `~/.mops/workspace/tasks/`) and attach it to `InternalAgentAPI`
