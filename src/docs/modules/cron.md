...
- `MOPS_CHAT_ID` -- current chat ID (set when `chat_id` is non-zero)
- `MOPS_TOPIC_ID` -- current topic ID (set when `topic_id` is non-None)
- `MOPS_TRANSPORT` -- transport identifier (`"tg"` or `"mx"`)
