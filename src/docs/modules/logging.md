...
- `mops_bot/log_context.py`: `ContextVar` state (`agent_name`, `operation`, `chat_id`, `session_id`) + `ContextFilter`.
- `mops_bot/logging_config.py`: root logger setup (console + rotating file via queue listener).
...
- rotating file logs in `~/.mops/logs/agent.log` (`5MB`, `3` backups)
